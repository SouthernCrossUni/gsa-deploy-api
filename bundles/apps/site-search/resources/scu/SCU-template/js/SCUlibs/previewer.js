...
(function($) {
  ...
  function customPositioner(viewer, parent) {
    ...
    var para = $parent.find('p:first');
    var para_offset = para.offset();
    ...
    // Choose horizontal position relative to para
    var left = para.width() + para_offset.left - parent_offset.left;
    ...
    if (right_edge > viewport_width) {
      left -= Math.min(right_edge - viewport_width, para.width());
    }
