...
<!-- **********************************************************************
 Global Style variables (can be customized): '' for using browser's default
     ...
     ********************************************************************** -->
<xsl:variable name="global_font">"Roboto", "Helvetica Neue", Helvetica, Arial, sans-serif</xsl:variable>
...
<xsl:variable name="hideYear" select="'2019'"/>
...
<xsl:template name="reformat_keyword">
  ...
  <xsl:choose>
   ...
   <!--<div style="background: #adadad;">    
       ...
       <xsl:variable name="webYear" select="MT[@N = 'courses.year']/@V"/>
       <xsl:variable name="spkCode" select="MT[@N = 'courses.spk-code']/@V"/>
        ...
        <div class="dom-avails avail-table">
          <!--RAW-DOM:<xsl:value-of select="MT[@N = 'courses.avail-combo']/@V"/>-->
           <xsl:call-template name="transformAvailCombo">
               ...
               <xsl:with-param name="courseUrl" select="U"/>
               <xsl:with-param name="hideMe" select="$hideMe"/>
           </xsl:call-template>
        </div>
        <div class="int-avails avail-table">
          <!-- hide the int avails if its a hide year so we dont produce two duplicate rows -->
...
<xsl:template name="getAvailabilities">
 ...
 <xsl:template name="transformAvailCombo">
      ...
      <xsl:param name="webYear" />
      <xsl:param name="spkCode" />
      <xsl:param name="courseUrl" />
      <xsl:param name="hideMe" />
...
<!--       <xsl:value-of select="$resSemTypeOne" />
      ...
      <xsl:if test="(not($resSemTypeOne/availability) and not($resSemTypeTwo/availability) and not($sesTypes/availability) and not($stPrdTypes/availability) and not($ylType/availability) or ($hideMe = 'true'))">
        <xsl:variable name="prevUrl"><xsl:value-of select="concat(substring($courseUrl, 1, string-length($courseUrl) - 4), number($hideYear)-1)"/></xsl:variable>
        <xsl:variable name="noAvailMessage">
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:variable name="notOfferedYear" select="number($hideYear)-2"/>
              <xsl:variable name="notCurrentlyAvailYear" select="number($hideYear)-1"/>
              <xsl:choose>
                <xsl:when test="$webYear = $notOfferedYear">
                  Not offered in <xsl:value-of select="number($webYear)"/>
                </xsl:when>
                <xsl:when test="$webYear = $notCurrentlyAvailYear">
                  Not currently available in <xsl:value-of select="number($webYear)"/>
                </xsl:when>
                <xsl:otherwise>
       ...
       <table cellspacing="0">
        ...
        <tbody>
          <td colspan="3"><strong><xsl:copy-of select="$noAvailMessage"/></strong></td>
        </tbody>
      </table>
      </xsl:if>
   </xsl:template>
