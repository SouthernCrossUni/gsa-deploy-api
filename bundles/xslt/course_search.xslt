  ...
  <xsl:choose>
   ...
   <!--<div style="background: #adadad;">    
        ...
        <div class="dom-avails avail-table">
          ...
          <xsl:choose>
            <xsl:when test="/GSP/PARAM[(@name='site') and(@value='squiz-courses')]">
              <xsl:call-template name="avail-info">
                <xsl:with-param name="avails" select="MT[@N = 'courses.avail-combo']/@V"/>
            ...
            <xsl:otherwise>
              <xsl:call-template name="avail-info">
                <xsl:with-param name="avails" select="MT[@N = 'courses.avail-combo']/@V"/>
                <xsl:with-param name="headTitle" select="'Intakes'"/>
              </xsl:call-template>
            </xsl:otherwise>
          </xsl:choose>
        </div>
        <div class="int-avails avail-table">
          ...
          <xsl:call-template name="avail-info">
            <xsl:with-param name="avails" select="MT[@N = 'courses.int-avail-combo']/@V"/>
...
<xsl:template name="avail-info-OLD">
  ...
  <table cellspacing="0">
    ...
    <tr>
      ...
      <td>
        <xsl:call-template name="split-avail">
          <xsl:with-param name="pText" select="$avails"/>
          <xsl:with-param name="pDelim" select="'CH;'"/>
        </xsl:call-template>  
      </td>
      <td>
        <xsl:call-template name="split-avail">
          <xsl:with-param name="pText" select="$avails"/>
          <xsl:with-param name="pDelim" select="'GCB;'"/>
        </xsl:call-template>  
      </td>
      <td>
        <xsl:call-template name="split-avail">
          <xsl:with-param name="pText" select="$avails"/>
          <xsl:with-param name="pDelim" select="'L;'"/>
        </xsl:call-template>  
      </td>
      <td>
        <xsl:call-template name="split-avail">
          <xsl:with-param name="pText" select="$avails"/>
          <xsl:with-param name="pDelim" select="'U;'"/>
        </xsl:call-template>  
      </td>
    </tr>
  </table>
...
<!-- pText: ;;CH;11;;L;11;;GC;12;;L;13;; -->
<!-- pDelim: L; -->
<!-- output: 1, 3 -->
<xsl:template name="split-avail-OLD">
  <xsl:param name="pText"/>
  <xsl:param name="pDelim"/>

  <xsl:if test="string-length($pText) and contains($pText, $pDelim)">
    <!-- store everything after first match in vTmp -->
    <xsl:variable name="vTmp" select="substring-after($pText, $pDelim)"/>
    <!-- display the session number for this match -->
    <xsl:value-of select="substring($vTmp,2,1)"/>
    <!-- if more matches are to follow, show a comma and call this template again -->
    <xsl:if test="string-length($vTmp) and contains($vTmp, $pDelim)">
      <xsl:text>, </xsl:text>
      <xsl:call-template name="split-avail">
        <xsl:with-param name="pText" select="$vTmp"/>
...
<xsl:template name="avail-info">
  <xsl:param name="avails"/>
  <xsl:param name="headTitle"/>
  <xsl:choose>
    <xsl:when test="contains($avails, ';OL;2')">
  
      <table cellspacing="0">
       <tbody>
        <tr>
          <td><xsl:value-of select="$headTitle"/></td>
          <td><span style="font-size: 0.9em">Available in Study Period</span></td>
        </tr>
      <xsl:if test="contains($avails, ';OL;')">
        <tr>
          <td><strong>SCU Online</strong></td>
            <xsl:call-template name="split-avail-online">
              <xsl:with-param name="pText" select="$avails"/>
              <xsl:with-param name="pDelim" select="';OL;'"/>
            </xsl:call-template>  
        </tr>
      </xsl:if>
       </tbody>
    ...
    <xsl:otherwise>
      <table cellspacing="0">
       <tbody>
      <xsl:choose>
       <xsl:when test="matches($avails,';[A-Z]+;')">
        <tr>
          <td><xsl:value-of select="$headTitle"/></td>
      ...
      <xsl:if test="contains($avails, ';CH;')">
        <tr>
          <td><strong>Coffs Harbour</strong></td>
            <xsl:call-template name="split-avail">
              <xsl:with-param name="pText" select="$avails"/>
              <xsl:with-param name="pDelim" select="';CH;'"/>
            </xsl:call-template>  
      ...
      <xsl:if test="contains($avails, ';GCB;')">
        <tr>
      ...
      <xsl:if test="contains($avails, ';MSC;')">
        <tr>
          <td><strong>National Marine Science Centre</strong></td>
            <xsl:call-template name="split-avail">
              <xsl:with-param name="pText" select="$avails"/>
              <xsl:with-param name="pDelim" select="';MSC;'"/>
            </xsl:call-template>  
        </tr>
      </xsl:if>
      <xsl:if test="contains($avails, ';SYD;')">
        <tr>
          <td><strong>Sydney</strong></td>
            <xsl:call-template name="split-avail">
              <xsl:with-param name="pText" select="$avails"/>
              <xsl:with-param name="pDelim" select="';SYD;'"/>
            </xsl:call-template>  
        </tr>
      </xsl:if>
      <xsl:if test="contains($avails, ';S;')">
        <tr>
          <td><strong>The Hotel School Sydney</strong></td>
            <xsl:call-template name="split-avail">
              <xsl:with-param name="pText" select="$avails"/>
              <xsl:with-param name="pDelim" select="';S;'"/>
            </xsl:call-template>  
        </tr>
      </xsl:if>
      </xsl:when>
...
<!-- pDelim: L; -->
<!-- output: 1, 3 -->
<xsl:template name="split-avail">
  <xsl:param name="pText"/>
  <xsl:param name="pDelim"/>

  <xsl:if test="string-length($pText) and contains($pText, $pDelim)">
    <xsl:variable name="delim1" select="concat($pDelim, '11;')"/>
    <xsl:variable name="delim2" select="concat($pDelim, '12;')"/>
    <xsl:variable name="delim3" select="concat($pDelim, '13;')"/>
    <!-- display the session numbers for this match -->
    <td> <xsl:if test="contains($pText, $delim1)">&#10003;</xsl:if> </td>
    <td> <xsl:if test="contains($pText, $delim2)">&#10003;</xsl:if> </td>
    <td> <xsl:if test="contains($pText, $delim3)">&#10003;</xsl:if> </td>
  </xsl:if>
</xsl:template>

<xsl:template name="split-avail-online">
  <xsl:param name="pText"/>
  <xsl:param name="pDelim"/>

  <xsl:if test="string-length($pText) and contains($pText, $pDelim)">
    <xsl:variable name="delim1" select="concat($pDelim, '21;')"/>
    <xsl:variable name="delim2" select="concat($pDelim, '22;')"/>
    <xsl:variable name="delim3" select="concat($pDelim, '23;')"/>
    <xsl:variable name="delim4" select="concat($pDelim, '24;')"/>
    <xsl:variable name="delim5" select="concat($pDelim, '25;')"/>
    <xsl:variable name="delim6" select="concat($pDelim, '26;')"/>
    
    <xsl:variable name="outtext">
      <xsl:if test="contains($pText, $delim1)">1, </xsl:if>
      <xsl:if test="contains($pText, $delim2)">2, </xsl:if>
      <xsl:if test="contains($pText, $delim3)">3, </xsl:if>
      <xsl:if test="contains($pText, $delim4)">4, </xsl:if>
      <xsl:if test="contains($pText, $delim5)">5, </xsl:if>
      <xsl:if test="contains($pText, $delim6)">6, </xsl:if>
    </xsl:variable>

    <td><xsl:value-of select="substring($outtext, 0, string-length($outtext) - 1)" /></td>
